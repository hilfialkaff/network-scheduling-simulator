...
public class MRBench extends Configured implements Tool{
  ...
  private ArrayList<Long> runJobInSequence(JobConf masterJobConf, int numRuns) throws IOException {
    ...
    ArrayList<JobID> jobs_id = new ArrayList<JobID>();

    HashMap begTimes = new HashMap();
    HashMap endTimes = new HashMap();
    
    JobStatus[] jobs_status;
    for (int i = 0; i < numRuns; i++) {
      ...
      // JobID id = jobConf.get("mapred.job.id");
      // jobs_id.add(id);
      // begTimes.put(id, curTime);
      // JobClient.runJob(jobConf);
      jc.submitJob(jobConf);
      // execTimes.add(new Long(System.currentTimeMillis() - curTime));

      jobs_status = jc.getAllJobs();
      System.out.println("submitted jobs: " + jobs_status.length);
    }
    ...
    do {
        ...
        // long curTime = System.currentTimeMillis();
        for (JobStatus js : jobs_status) {
            if (js.isJobComplete()) {
                count++;
            }
            // endTimes.put(js.getJobID(), curTime);
        }

        System.out.println("finished jobs: " + jobs_status.length);
    } while (count == jobs_status.length);
  ...
  public int run(String[] args) throws Exception {
    ...
    double stdDev = 0;
    for(Long time : execTimes) {
        temp += (avgTime-time) * (avgTime-time);
    }

    stdDev = Math.sqrt(temp/numRuns);

    // System.out.println("DataLines\tMaps\tReduces\tAvgTime (milliseconds)");
    // System.out.println(inputLines + "\t\t" + numMaps + "\t" + 
    //                    numReduces + "\t" + avgTime);

    System.out.println("Mappers: " + numMaps);
    System.out.println("Reducers: " + numReduces);
    System.out.println("Avg (ms): " + avgTime);
    System.out.println("Stddev (ms): " + stdDev);
    */
